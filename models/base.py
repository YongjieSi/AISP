...
class BaseLearner(object):
    ...
    def feature_dim(self):
        if isinstance(self._network, nn.DataParallel):
            return self._network.module._feature_dim
        else:
            return self._network._feature_dim
    ...
    def eval_task(self,layers=None):
        ...
        if self.args["model_name"] == "adam_memo":
            ...
            y_pred, y_true = self._eval_cnn(self.test_loader,layers)
    ...
    def _compute_accuracy_ssl(self, model, loader):
        model.eval()
        correct, total = 0, 0
        for i, (_, inputs, targets) in enumerate(loader):
            inputs = inputs.to(self._device)
            with torch.no_grad():
                acc,loss = model(inputs)
            # predicts = torch.max(outputs, dim=1)[1]
            # correct += (predicts.cpu() == targets).sum()
            # total += len(targets)
    ...
    def _compute_accuracy_new(self, model, loader):
        model.eval()
        correct, total = 0, 0
        for i, (_, inputs, targets) in enumerate(loader):
            inputs = inputs.to(self._device)
            with torch.no_grad():
                outputs = model.forward_base(inputs)["logits"]
            predicts = torch.max(outputs, dim=1)[1]
            correct += (predicts.cpu() == targets).sum()
            total += len(targets)
    ...
    def _eval_adam(self, loader):
        self._network.eval()
        y_pred, y_true = [], []
        for _, (_, inputs, targets) in enumerate(loader):
            inputs = inputs.to(self._device)
            with torch.no_grad():
                outputs = self._network._forward(inputs)["logits"]
            predicts = torch.topk(
    ...
    def _eval_cnn(self, loader,layers):
        self._network.eval()
        y_pred, y_true = [], []
        for _, (_, inputs, targets) in enumerate(loader):
            inputs = inputs.to(self._device)
    ...
    def _eval_mine(self, loader,task):
        test_class = self.args['init_cls'] + task * self.args['increment']
        ...
        for _, (_, inputs, targets) in enumerate(loader):
            inputs = inputs.to(self._device)
            with torch.no_grad():
                outputs = self._network(inputs)["logits"][:, :test_class]
            predicts = torch.topk(
                outputs, k=self.topk, dim=1, largest=True, sorted=True
            )[
                1
    ...
    def _construct_exemplar_unified(self, data_manager, m):
        ...
        for class_idx in range(self._known_classes):
            ...
            class_dset = data_manager.get_dataset(
                [], source="train", mode="base", appendent=(class_data, class_targets)
            )
