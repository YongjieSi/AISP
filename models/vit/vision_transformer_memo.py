...
from models.vit.layers import DropPath, to_2tuple, trunc_normal_
...
# override the timm PatcchEmbed to relax the input shape constraint.
...
class Specialized_Vit(VisionTransformer):
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=0, global_pool='token', embed_dim=768, depth=12, num_heads=12, mlp_ratio=4, qkv_bias=True, init_values=None, class_token=True, no_embed_class=False, pre_norm=False, fc_norm=None, drop_rate=0, attn_drop_rate=0, drop_path_rate=0, weight_init='', embed_layer=PatchEmbed, norm_layer=None, act_layer=None, block_fn=Block):
        ...
        super().__init__(img_size, patch_size, in_chans, num_classes, embed_dim, depth, num_heads, mlp_ratio, qkv_bias, drop_rate, attn_drop_rate, drop_path_rate, hybrid_backbone=None, norm_layer=None)
        norm_layer = norm_layer or partial(nn.LayerNorm, eps=1e-6)
...
def _create_vision_transformer_memo(variant, pretrained=False, distilled=False, **kwargs):
    default_cfg = default_cfgs[variant]
    default_num_classes = default_cfg['num_classes']
    default_img_size = default_cfg['input_size'][-1]
    ...
    if repr_size is not None and num_classes != default_num_classes:
        ...
        _logger.warning("Removing representation layer for fine-tuning.")
        repr_size = None
    # model_cls = DistilledVisionTransformer if distilled else Specialized_Vit

    model_cls = Specialized_Vit
...
def vit_deit_base_distilled_patch16_384_memo(pretrained=False, **kwargs):
    """ DeiT-base distilled model @ 384x384 from paper (https://arxiv.org/abs/2012.12877).
    ImageNet-1k weights from https://github.com/facebookresearch/deit.
    """
    model_kwargs = dict(patch_size=16, embed_dim=768, depth=12, num_heads=12, **kwargs)
    model = _create_vision_transformer_memo(
        'vit_deit_base_distilled_patch16_384', pretrained=pretrained, distilled=True, **model_kwargs)
    return model


if __name__ == "__main__":
    model = vit_deit_tiny_distilled_patch16_224(pretrained=True)
    print(model)